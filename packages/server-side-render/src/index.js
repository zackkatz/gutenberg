 ...
 */
import ServerSideRender, { useServerSideRender } from './server-side-render';
...
const exportedUseServerSideRender = withSelect( ( select ) => {
	// FIXME: @wordpress/server-side-render should not depend on @wordpress/editor.
	// It is used by blocks that can be loaded into a *non-post* block editor.
	// eslint-disable-next-line @wordpress/data-no-store-string-literals
	const coreEditorSelect = select( 'core/editor' );
...
if ( window && window.wp && window.wp.components ) {
	window.wp.components.ServerSideRender = forwardRef( ( props, ref ) => {
		...
		return useServerSideRender( { ...props, ref } );
	} );
}

export { exportedUseServerSideRender as useServerSideRender };
